...
def getMeanAndVolReturns(prices):
    priceArr = np.array(prices)
    stockReturns = priceArr[1:len(prices)] - priceArr[0:len(prices)-1]
...
stdDevs = {}
for symbol in data:
    means[symbol], variances[symbol], stdDevs[symbol] = getMeanAndVolReturns(data[symbol])
